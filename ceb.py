...
def solve(tirage_nombres,nombre_a_trouver,printer=False,return_one_solution=False):
    try:
        tt=time.time()
        def info(txt):
        ...
        nts=[]
        evc=[]
        nombres=tirage_nombres
        nat=nombre_a_trouver
        if nat in nombres:
            s=str(nat)+"="+str(nat)
        ...
        while True:
            ...
            for i in calculs:
                ...
                for cn1 in inombres:
                    ...
                    inombres2=inombres.copy()
                    inombres2.remove(cn1)
                    for cn2 in inombres2:
                        if cn1<cn2:
                            if c1:
                                continue
                        inombres3=inombres2.copy()
                        inombres3.remove(cn2)
                        for op in OPS:
                            resultat=calculate(cn1,op,cn2)
                            if int(resultat)!=resultat:
                                if c2:
                                    continue
                            resultat=int(resultat)
                                
                            new_calcul=icalculs+[str(cn1)+op+str(cn2)+"="+str(resultat)]
                            nma=abs(resultat-nat)
                            new_calculs.append((new_calcul,inombres3+[resultat]))
                            if nma<=m_a:
                                if nma<m_a:
                                    solutions.clear()
                                    nts.clear()
                                ...
                                if resultat not in nts:
                                    nts.append(resultat)
                                if new_calcul in solutions:
                                    if c3:
                                        continue
                                nc=" ".join(new_calcul)
                                solutions.append(new_calcul)
                                if m_a:
                                    info(""+str(resultat)+": "+str(nc))
                                else:
                                    info("Le Compte Est Bon : "+str(nc))
                                    if return_one_solution:
                                        return (solutions,nts)
                            evcal.append(new_calcul)
            if not m_a:
                if 0 and c4:
                    return (solutions,nts)
            calculs=new_calculs
