                    ...
                    for in2 in range(l-one):
                        ...
                        for _ in range(one):
                            ...
                            for op in OPS:
                                ...
                                i=int(resultat)
                                if not allow_float and i!=resultat:continue
                                #if not (allow_float or i==resultat):continue
                                resultat=i
                                cal="%s%s%s=%s"%(n1,op,n2,resultat)
                                #cal=str(n1)+op+str(n2)+equal+str(resultat)
                                #ncals=cals#+[cal]
                                ncals=cals+[cal]
                                nma=abs(resultat-nat)
                                npistes.append((ncals,nbr+[resultat]))
                                if nma<=m_a:
                                    if nma<m_a:
                                        solutions.clear()
                                        nts.clear()
