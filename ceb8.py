...
calfmt="%s%s%s=%s"
ncalsfmt="%s %s"
def solve(nombres,nat,allow_float=False,avoid_doubles=True):
    
    if nat in nombres:yield (nat,"%s=%s"%(nat,nat))
    pistes=[("",sorted(nombres,reverse=True))]
    solutions=[]
    m_a=nat
    while True:
        npistes=[]
            ...
            for in1 in range(l):
                ...
                for in2 in range(l-one):
                    ...
                    for op in OPS:
                        ...
                        i=int(resultat)
                        if i==resultat:resultat=i
                        elif not allow_float:continue
                        cal=calfmt%(n1,op,n2,resultat)
                        ncals=ncalsfmt%(cals,cal)
                        nma=abs(resultat-nat)
                        #try:i=nbr3.index(resultat)
                        ...
                        if nma<=m_a:
                            if nma<m_a:
                                solutions.clear()
                                m_a=nma
                            if avoid_doubles and ncals in solutions:
                                #print("MY DOUBLION %s"%ncals)
                                continue
...
def test():
    ...
    print(time.time()-tt)
#tt=time.time()
def atest(nombres=[8, 10, 75, 5, 100, 3],nat=646):
    tt=time.time()
    try:
        for result in solve(nombres,nat):
...
"""
atest()
#atest([100, 100, 100, 100, 100, 100],600)
#atest([6, 1, 10, 2, 5, 3],124)
