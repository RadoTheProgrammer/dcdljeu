...
FILE_TLM=os.path.join(os.path.dirname(__file__),"tlm_dcdl.txt")
FILE_TLM_SORT=os.path.join(os.path.dirname(__file__),"tlm_dcdl_sort.txt")
...
BS=[]
ID_BS=[]
FILE_BS=os.path.join(os.path.dirname(__file__),"bs.txt")
f=open(FILE_BS,"rb")
for b in f.read():
    BS.append(bytes([b]))
    ID_BS.append(b)
